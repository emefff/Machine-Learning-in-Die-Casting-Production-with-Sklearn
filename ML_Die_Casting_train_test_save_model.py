...
def scrap_due_to_low_values(dataframe, limits_list, set_values_list):
    """
    Low values of three process parameters (velocity_phase2, pressure_packing, temperature_die)
    ...
    factor_velocity_phase2 = limits_list[0]
    factor_press_packing = limits_list[1]
    ...
    for i in range(NUMBER_OF_SHOTS):
        ...
        if df["velocity_phase2"][i] < factor_velocity_phase2 * velocity_phase2_set:
            part_ok = 0
            num_parts_scrap_vphase2 += 1
        else:
            part_ok = 1
        # packing pressure low --> insufficient filling of part
        if df["pressure_packing"][i] < factor_press_packing * pressure_packing_set:
            part_ok = 0
...
NUMBER_OF_SHOTS = int(num_parts_per_day * 2) #
print(f"{NUMBER_OF_SHOTS = }\n")

velocity_phase2_list = []
pos_shot_start_list = []
pos_shot_end_list = []
...
velocity_phase2_list = np.random.normal(loc=VELOCITY_PHASE2_SET, scale=VELOCITY_PHASE2_DEVIATION, size=NUMBER_OF_SHOTS) # mm/s
...
df = pd.DataFrame({'velocity_phase2' : velocity_phase2_list})
...
percentage_limit_velocity_phase2 = 0.95
percentage_limit_press_packing = 0.875
temperature_limit_die = 240

limits_list = [percentage_limit_velocity_phase2, percentage_limit_press_packing, temperature_limit_die]
...
num_parts_affected = int(0.015 * NUMBER_OF_SHOTS)
PRESS_PHASE2_FACTOR_LIST = [np.random.uniform(0.2, 0.9) for i in range(num_parts_affected)] # generate some values < 1 for short shots
...
num_parts_affected = int(0.015 * NUMBER_OF_SHOTS)
VOLUME_FACTOR_LIST = [np.random.uniform(0.2, 0.98) for i in range(num_parts_affected)] # generate some values < 1 for short shots
...
if plots == 'yes': # A LOT OF PLOTS!
    ...
    plt.hist(velocity_phase2_list, bins=BINS, label='velocity_phase2')
    plt.legend()
    plt.xlabel('velocity_phase2 / mm/s')
...
plt.figure(figsize = (15,9))
sns.heatmap(df.corr(), cmap = "magma", annot = True, vmin=0, vmax=0.5) # inferno
plt.show()
...
print(f"{rf_model.max_depth = }")
print(f"{rf_model.max_features = }")
print("")
...
n_cores_half = n_cores//2
print(f"{n_cores = }")
print(f"{n_cores_half = }")
...
print(50*"*")
print("\nGridSearchCV........\n")
rf_cv_model = GridSearchCV(rf_model2, rf_params, cv = 5, n_jobs = 4) # oder -1 für alle n_cores
# cv was 5

rf_cv_model.fit(X_train, y_train)

print(f"{rf_cv_model.best_params_ = }")
max_feat = rf_cv_model.best_params_["max_features"]
n_estim = rf_cv_model.best_params_["n_estimators"]
min_sam_split = rf_cv_model.best_params_["min_samples_split"]

rf_tuned = RandomForestClassifier(max_features=max_feat,
