...
make_prediction(np.array([4000, 425, 50, 250, 636172.5]).reshape(1, -1))

# now we test a scrap part (velocity too low...)
make_prediction(np.array([3750, 425, 50, 250, 636172.5]).reshape(1, -1))

# now we test a scrap part (velocity ok, but packing_pressure very low...)
make_prediction(np.array([3850, 300, 50, 250, 636172.5]).reshape(1, -1))

# now we test a scrap part (velocity ok, packing_pressure ok, die temp very low...)
make_prediction(np.array([3850, 425, 50, 220, 636172.5]).reshape(1, -1))
    
# now we test a scrap part (pressure phase 2 very low)
make_prediction(np.array([3850, 425, 10, 255, 636172.5]).reshape(1, -1))

# now we test a scrap part (short shot)
make_prediction(np.array([3850, 425, 50, 255, 0.8*636172.5]).reshape(1, -1))

# now we test a scrap part (short shot)
make_prediction(np.array([3850, 425, 50, 255, 0.9*636172.5]).reshape(1, -1))

# now we test a scrap part (short shot)
make_prediction(np.array([3850, 425, 50, 255, 0.97*636172.5]).reshape(1, -1))
